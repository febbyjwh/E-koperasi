@extends('layout')

@section('title', 'Identitas Diri')

@section('content')
<!-- component -->
<!-- This is an example component -->
{{-- <div class="p-6 bg-white rounded-2xl shadow-lg w-full"> --}}
<div class="h-screen">
 
	<link rel="stylesheet" href="https://unpkg.com/tailwindcss@^1.0/dist/tailwind.min.css">
	<script src="https://cdn.jsdelivr.net/gh/alpinejs/alpine@v2.x.x/dist/alpine.js" defer></script>

	<style>
		[x-cloak] {
			display: none;
		}

		[type="checkbox"] {
			box-sizing: border-box;
			padding: 0;
		}

		.form-checkbox,
		.form-radio {
			-webkit-appearance: none;
			-moz-appearance: none;
			appearance: none;
			-webkit-print-color-adjust: exact;
			color-adjust: exact;
			display: inline-block;
			vertical-align: middle;
			background-origin: border-box;
			-webkit-user-select: none;
			-moz-user-select: none;
			-ms-user-select: none;
			user-select: none;
			flex-shrink: 0;
			color: currentColor;
			background-color: #fff;
			border-color: #e2e8f0;
			border-width: 1px;
			height: 1.4em;
			width: 1.4em;
		}

		.form-checkbox {
			border-radius: 0.25rem;
		}

		.form-radio {
			border-radius: 50%;
		}

		.form-checkbox:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M5.707 7.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L7 8.586 5.707 7.293z'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}
		
		.form-radio:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}
	</style>

	<div x-data="app()" x-cloak>
		<div class="max-w-3xl mx-auto px-4 py-10">

			<div x-show.transition="step === 'complete'">
				<div class="bg-white rounded-lg p-10 flex items-center shadow justify-between">
					<div>
						<svg class="mb-4 h-20 w-20 text-green-500 mx-auto" viewBox="0 0 20 20" fill="currentColor">  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>

						<h2 class="text-2xl mb-4 text-gray-800 text-center font-bold">Berhasil Mengisi Identitas</h2>

						<div class="text-gray-600 mb-8">
							Terima kasih telah mengisi identitas ini dengan benar, silahkan mengajukan pinjaman >-<.
						</div>

						<a href="{{ route('anggota.anggota') }}" 
                        class="w-40 block mx-auto focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border rounded-lg"
                        >
                        &lt;- Kembali
                        </a>
					</div>
				</div>
			</div>

			<div x-show.transition="step != 'complete'">	
				<!-- Top Navigation -->
				<div class="border-b-2 py-4">
					<div class="uppercase tracking-wide text-xs font-bold text-gray-500 mb-1 leading-tight" x-text="`Langkah: ${step} dari 3"></div>
					<div class="flex flex-col md:flex-row md:items-center md:justify-between">
						<div class="flex-1">
							<div x-show="step === 1">
								<div class="text-lg font-bold text-gray-700 leading-tight">Data Pribadi</div>
							</div>
							
							<div x-show="step === 2">
								<div class="text-lg font-bold text-gray-700 leading-tight">Data Pekerjaan/Keangotaan</div>
							</div>

							<div x-show="step === 3">
								<div class="text-lg font-bold text-gray-700 leading-tight">Data Keluarga</div>
							</div>
						</div>

						<div class="flex items-center md:w-64">
							<div class="w-full bg-white rounded-full mr-2">
								<div class="rounded-full bg-green-500 text-xs leading-none h-2 text-center text-white" :style="'width: '+ parseInt(step / 4 * 100) +'%'"></div>
							</div>
							<div class="text-xs w-10 text-gray-600" x-text="parseInt(step / 4 * 100) +'%'"></div>
						</div>
					</div>
				</div>
				<!-- /Top Navigation -->

				<!-- Step Content -->
				<div class="mb-15 mt-5">	
					<!-- Step 1: Data Pribadi -->
                    <div x-show.transition.in="step === 1" class="bg-white p-6 rounded-lg shadow-md">
                        
                        <h2 class="text-xl font-bold text-gray-800 mb-6 text-center">Data Pribadi</h2>

                        <!-- Foto KTP -->
                        <div class="mb-6 text-center">
                            <div class="mx-auto" style="width: 323px; height: 204px; border: 1px solid #ccc; overflow: hidden; border-radius: 8px; background: #f3f4f6;">
                            <img id="image" 
                                class="object-cover w-full h-full" 
                                :src="image" 
                                style="width: 323px; height: 204px;" />
                            </div>

                            <label 
                                for="fileInput"
                                class="mt-3 cursor-pointer inline-flex items-center border px-4 py-2 rounded-lg shadow-sm bg-white hover:bg-gray-100 text-gray-600 font-medium"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" 
                                    class="w-5 h-5 mr-2 text-gray-500" 
                                    fill="none" viewBox="0 0 24 24" 
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                        d="M5 7h1a2 2 0 002-2 1 1 0 011-1h6a1 1 0 011 1 2 2 0 002 2h1a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9a2 2 0 012-2z" />
                                    <circle cx="12" cy="13" r="3" stroke="currentColor" stroke-width="2"/>
                                </svg>
                                Pilih Foto KTP
                            </label>

                            <input type="file" id="fileInput" accept="image/*" class="hidden"
                                @change="let file = $event.target.files[0]; 
                                            if(file){
                                                let reader = new FileReader();
                                                reader.onload = e => image = e.target.result;
                                                reader.readAsDataURL(file);
                                            }">
                            
                            <p class="text-xs text-gray-500 mt-2">Klik untuk mengunggah foto KTP</p>
                        </div>

                        <!-- Form Fields -->
                        <div class="space-y-5">
                            <div>
                                <label for="namalengkap" class="font-bold text-gray-700 block mb-1">Nama Lengkap</label>
                                <input id="namalengkap" type="text" placeholder="Masukan nama lengkap..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="nik" class="font-bold text-gray-700 block mb-1">Nomber Induk Kependudukan (NIK)</label>
                                <input id="nik" type="text" placeholder="Masukan nomber induk kependudukan..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="username" class="font-bold text-gray-700 block mb-1">Nama Pengguna</label>
                                <input id="username" type="text" placeholder="Masukan nama pengguna..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="ulangtahun" class="font-bold text-gray-700 block mb-1">Tanggal Lahir</label>
                                <input id="ulangtahun" type="date"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="gender" class="font-bold text-gray-700 block mb-1">Jenis Kelamin</label>
                                <select id="gender"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                                    <option value="">Pilih jenis kelamin</option>
                                    <option value="Laki-laki">Laki-laki</option>
                                    <option value="Perempuan">Perempuan</option>
                                </select>
                            </div>

                            <div>
                                <label for="email" class="font-bold text-gray-700 block mb-1">Email</label>
                                <input id="email" type="email" placeholder="Masukan alamat email..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="nomor" class="font-bold text-gray-700 block mb-1">Nomor WhatsApp</label>
                                <input id="nomor" type="text" placeholder="Masukan nomor WhatsApp..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="alamat" class="font-bold text-gray-700 block mb-1">Alamat KTP</label>
                                <textarea id="alamat" rows="3" placeholder="Masukan alamat sesuai KTP..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700"></textarea>
                            </div>
                        </div>
                    </div>

					<div x-show.transition.in="step === 2" class="bg-white p-6 rounded-lg shadow-md relative z-0">

						<div class="space-y-5">
                            <div>
                                <label for="noanggota" class="font-bold text-gray-700 block mb-1">No Anggota</label>
                                <input id="noanggota" type="text" placeholder="Masukan nomor anggota..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="nip" class="font-bold text-gray-700 block mb-1">NIP</label>
                                <input id="nip" type="text" placeholder="Masukan NIP..."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="jabatan" class="font-bold text-gray-700 block mb-1">Jabatan / Posisi</label>
                                <input id="jabatan" type="text" placeholder="Contoh: Guru Matematika, Staff Tata Usaha, Kepala Sekolah, dll."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="unitkerja" class="font-bold text-gray-700 block mb-1">Unit Kerja / Departemen</label>
                                <input id="unitkerja" type="text" placeholder="Contoh: Tata Usaha, Kurikulum, Kesiswaan, Laboratorium."
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="tglmulai" class="font-bold text-gray-700 block mb-1">Tanggal Mulai Kerja</label>
                                <input id="tglmulai" type="date"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="statuskepegawaian" class="font-bold text-gray-700 block mb-1">Status Kepegawaian</label>
                                <select id="statuskepegawaian"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                                    <option value="">Pilih status kepegawaian</option>
                                    <option value="PNS">PNS</option>
                                    <option value="Honorer">Honorer</option>
                                    <option value="Guru Tetap">Guru Tetap</option>
                                    <option value="Guru Tidak Tetap">Guru Tidak Tetap</option>
                                </select>
                            </div>

                            <div>
                                <label for="tglbergabung" class="font-bold text-gray-700 block mb-1">Tanggal Bergabung</label>
                                <input id="tglbergabung" type="date"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                            </div>

                            <div>
                                <label for="statusanggota" class="font-bold text-gray-700 block mb-1">Status Keanggotaan</label>
                                <select id="statusanggota"
                                    class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                                    <option value="">Pilih status keanggotaan</option>
                                    <option value="Aktif">Aktif</option>
                                    <option value="Nonaktif">Nonaktif</option>
                                </select>
                            </div>
                        </div>
					</div>

					<div x-show.transition.in="step === 3" class="bg-white p-6 rounded-lg shadow-md relative z-0">
						<div class="space-y-5">
                            <div>
                                <label for="namawali" class="font-bold text-gray-700 block mb-1">Nama Anggota Keluarga</label>
                                <input id="namawali" type="text" placeholder="Masukan nama anggota keluarga..."
                                class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700" />
                            </div>

                            <div>
                                <label for="hubunganwali" class="font-bold text-gray-700 block mb-1">Hubungan dengan Anggota</label>
                                <select id="hubunganwali"
                                class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700">
                                <option value="">Pilih hubungan</option>
                                <option value="Orang Tua">Orang Tua</option>
                                <option value="Saudara">Saudara</option>
                                <option value="Suami/Istri">Suami/Istri</option>
                                <option value="Anak">Anak</option>
                                <option value="Lainnya">Lainnya</option>
                                </select>
                            </div>

                            <div>
                                <label for="notelpwali" class="font-bold text-gray-700 block mb-1">Nomor Telepon Anggota Keluarga</label>
                                <input id="notelpwali" type="text" placeholder="Masukan nomor telepon anggota keluarga..."
                                class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700" />
                            </div>

                            <div>
                                <label for="alamatwali" class="font-bold text-gray-700 block mb-1">Alamat anggota keluarga</label>
                                <textarea id="alamatwali" rows="3" placeholder="Masukan alamat anggota keluarga..."
                                class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700"></textarea>
                            </div>

                            <div>
                                <label for="emailwali" class="font-bold text-gray-700 block mb-1">Email anggota keluarga (Opsional)</label>
                                <input id="emailwali" type="email" placeholder="Masukan email anggota keluarga..."
                                class="w-full px-4 py-3 rounded-lg border shadow-sm focus:outline-none focus:border-blue-500 text-gray-700" />
                            </div>
                            </div>

				</div>
				<!-- / Step Content -->
			</div>
		</div>

		<!-- Bottom Navigation -->	
		<div class="fixed bottom-0 left-0 right-0 py-5 bg-white shadow-md" x-show="step != 'complete'">
			<div class="max-w-3xl mx-auto px-4">
				<div class="flex justify-between">
					<div class="w-1/2">
						<button
							x-show="step > 1"
							@click="step--"
							class="w-32 focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border" 
						>Previous</button>
					</div>

					<div class="w-1/2 text-right">
						<button
							x-show="step < 3"
							@click="step++"
							class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium" 
						>Next</button>

						<button
							@click="step = 'complete'"
							x-show="step === 3"
							class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium" 
						>Complete</button>
					</div>
				</div>
			</div>
		</div>
		<!-- / Bottom Navigation https://placehold.co/300x300/e2e8f0/cccccc -->	
	</div>
{{-- </div> --}}
	<script>
		function app() {
			return {
				step: 1, 
				passwordStrengthText: '',
				togglePassword: false,

				image: 'data:image/jpeg;base64,',
				password: '',
				gender: 'Male',

				checkPasswordStrength() {
					var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
					var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");

					let value = this.password;

					if (strongRegex.test(value)) {
						this.passwordStrengthText = "Strong password";
					} else if(mediumRegex.test(value)) {
						this.passwordStrengthText = "Could be stronger";
					} else {
						this.passwordStrengthText = "Too weak";
					}
				}
			}
		}
	</script>
@endsection
