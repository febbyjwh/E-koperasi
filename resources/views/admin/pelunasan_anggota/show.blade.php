...
@section('title','Pelunasan Cicilan')
@section('content')
...
{{-- @include('components.alert') --}}
                    ...
                    @foreach ($pelunasans as $pelunasan)
                        ...
                        @if (auth()->user()->role === 'admin' || ($isOwner && auth()->user()->role === 'anggota'))
                            <tr class="hover:bg-gray-50">
                                ...
                                <td>{{ \Carbon\Carbon::parse($pelunasan->tanggal_jatuh_tempo)->translatedFormat('d F Y') }}
                                </td>
                                                        ...
                                                        <div class="p-4">
                                                            <form
                                                                ...
                                                                <div class="mb-4">
                                                                    ...
                                                                    <!-- Input untuk user -->
                                                                    <input type="text"
                                                                        id="displayJumlah{{ $pelunasan->id }}"
                                                                        value="Rp.{{ number_format($pelunasan->total_angsuran, 0, ',', '.') }}"
                                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5"
                                                                        oninput="formatRupiahInput(this, 'jumlahBayar{{ $pelunasan->id }}')"
                                                                        required />

                                                                    <!-- Input hidden untuk backend -->
                                                                    <input type="hidden" name="jumlah_bayar"
                                                                        id="jumlahBayar{{ $pelunasan->id }}"
            ...
            <div class="flex justify-end mt-4">
                @if (auth()->user()->role === 'admin')
                    <a href="{{ route('pelunasan_anggota.index') }}"
                        class="py-1 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                        Kembali
                    </a>
                @elseif(auth()->user()->role === 'anggota')
                    <a href="{{ route('cicilan_anggota.index') }}"
                        class="py-1 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                        Kembali
                    </a>
                @endif
    ...
    <script>
        function formatRupiahInput(el, hiddenId) {
            let angka = el.value.replace(/[^,\d]/g, ''); // hanya angka
            let number = angka.replace(/\B(?=(\d{3})+(?!\d))/g, '.'); // kasih titik ribuan
            el.value = 'Rp.' + number; // tampil di input user
            document.getElementById(hiddenId).value = angka; // simpan angka murni ke hidden input
        }
    </script>
